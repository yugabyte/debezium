...
public class YugabyteDBConnectionIT {
    ...
    public void shouldDetectRunningConncurrentTxOnInit() throws Exception {
        ...
        try (YugabyteDBConnection blockingConnection = TestHelper.create("blocker")) {
    ...
    private YugabyteDBConnection buildPG95PGConn(String name) {
        return new YugabyteDBConnection(TestHelper.defaultJdbcConfig().edit().with("ApplicationName", name).build()) {
            @Override
