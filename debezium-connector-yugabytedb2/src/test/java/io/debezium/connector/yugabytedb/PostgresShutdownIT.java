...
public class PostgresShutdownIT extends AbstractConnectorTest {
    ...
    public void setUp() {
        ...
        System.setProperty("database.port", String.valueOf(postgresContainer.getMappedPort(5433)));
        try {
    ...
    public void shouldStopOnPostgresFastShutdown() throws Exception {
        ...
        YugabyteDBConnection yugabyteDBConnection = TestHelper.create();
        String initialHeartbeat = yugabyteDBConnection.queryAndMap(
                "SELECT ts FROM s1.heartbeat;",
                yugabyteDBConnection.singleResultMapper(rs -> rs.getString("ts"), "Could not fetch keepalive info"));
        ...
        Awaitility.await()
                ...
                .until(() -> !initialHeartbeat.equals(yugabyteDBConnection.queryAndMap(
                        "SELECT ts FROM s1.heartbeat;",
                        yugabyteDBConnection.singleResultMapper(rs -> rs.getString("ts"), "Could not fetch keepalive info"))));
        logger.info("INTIAL Heartbeat: " + initialHeartbeat + " ; CURRENT heartbeat: "
                + yugabyteDBConnection.queryAndMap(
                        "SELECT ts FROM s1.heartbeat;",
                        yugabyteDBConnection.singleResultMapper(rs -> rs.getString("ts"), "Could not fetch keepalive info")));
