...
/**
 * Integration test for {@link YugabyteDBConnectorConfig.SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE}
 *
...
public class SnapshotWithOverridesProducerIT extends AbstractRecordsProducerTest {
    ...
    public void shouldUseOverriddenSelectStatementDuringSnapshotting() throws Exception {
        ...
        buildProducer(TestHelper.defaultConfig()
                .with(YugabyteDBConnectorConfig.SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE, "over.t1")
                .with(YugabyteDBConnectorConfig.SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE.name() + ".over.t1", "SELECT * FROM over.t1 WHERE pk > 100"));
    ...
    public void shouldUseMultipleOverriddenSelectStatementsDuringSnapshotting() throws Exception {
        ...
        buildProducer(TestHelper.defaultConfig()
                .with(YugabyteDBConnectorConfig.SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE, "over.t1,over.t2")
                .with(YugabyteDBConnectorConfig.SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE.name() + ".over.t1", "SELECT * FROM over.t1 WHERE pk > 101")
                .with(YugabyteDBConnectorConfig.SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE.name() + ".over.t2", "SELECT * FROM over.t2 WHERE pk > 100"));
