...
public class PostgresReplicationConnection extends JdbcConnection implements ReplicationConnection {
    ...
    private final YugabyteDBConnectorConfig.AutoCreateMode publicationAutocreateMode;
    private final YugabyteDBConnectorConfig.LogicalDecoder plugin;
    private final boolean dropSlotOnClose;
    private final YugabyteDBConnectorConfig originalConfig;
    ...
    private OpId defaultStartingPos;
     ...
     * @param yugabyteDBTypeRegistry              registry with PostgreSQL types
    ...
    // // CDCSDK: get the opid
    // final OpId xlogStart = queryAndMap(identifySystemStatement, rs -> {
    // if (!rs.next()) {
    ...
    // }
    // else {
    // OpId latestFlushedLsn = slotInfo.latestFlushedLsn();
    ...
    @Override
    public ReplicationStream startStreaming(OpId offset, WalPositionLocator walPosition) throws SQLException, InterruptedException {
        initConnection();
        ...
        int maxRetries = config().getInteger(YugabyteDBConnectorConfig.MAX_RETRIES);
        int delay = config().getInteger(YugabyteDBConnectorConfig.RETRY_DELAY_MS);
        int tryCount = 0;
        while (true) {
            try {
                // CDCSDK here we need to start calling the client
    ...
    private ReplicationStream createReplicationStream(final OpId startLsn, WalPositionLocator walPosition) throws SQLException, InterruptedException {
        ...
        // CDCSDK This is similar to the YBClient
        final YBClient ybClient;
        ...
        return new ReplicationStream() {
            ...
            private volatile OpId lastReceivedLsn;
            ...
            private void deserializeMessages(ByteBuffer buffer, ReplicationMessageProcessor processor) throws SQLException, InterruptedException {
                ...
                messageDecoder.processMessage(buffer, processor, yugabyteDBTypeRegistry);
            }
            ...
            @Override
            public void flushLsn(OpId lsn) throws SQLException {
                doFlushLsn(lsn);
            }

            private void doFlushLsn(OpId lsn) throws SQLException {
            ...
            public OpId startLsn() {
                return null;// startLsn;
            }
        };
    }

    private PGReplicationStream startPgReplicationStream(final OpId lsn, Function<ChainedLogicalStreamBuilder, ChainedLogicalStreamBuilder> configurator)
            throws SQLException {
        assert lsn != null;
    ...
    protected static void defaultSettings(Configuration.Builder builder) {
        ...
        YugabyteDBConnection.defaultSettings(builder);
    ...
    protected static class ReplicationConnectionBuilder implements Builder {
        ...
        private YugabyteDBConnectorConfig.AutoCreateMode publicationAutocreateMode = YugabyteDBConnectorConfig.AutoCreateMode.ALL_TABLES;
        private YugabyteDBConnectorConfig.LogicalDecoder plugin = YugabyteDBConnectorConfig.LogicalDecoder.DECODERBUFS;
        private YugabyteDBConnectorConfig.TruncateHandlingMode truncateHandlingMode;
        ...
        protected ReplicationConnectionBuilder(YugabyteDBConnectorConfig config) {
            assert config != null;
            this.config = config;
        }
        ...
        @Override
        public Builder withPublicationAutocreateMode(YugabyteDBConnectorConfig.AutoCreateMode publicationAutocreateMode) {
            assert publicationName != null;
            this.publicationAutocreateMode = publicationAutocreateMode;
            return this;
        }
        ...
        @Override
        public ReplicationConnectionBuilder withPlugin(final YugabyteDBConnectorConfig.LogicalDecoder plugin) {
            assert plugin != null;
            this.plugin = plugin;
            return this;
        }
        ...
        @Override
        public Builder withTruncateHandlingMode(YugabyteDBConnectorConfig.TruncateHandlingMode truncateHandlingMode) {
        ...
        @Override
        public Builder withTypeRegistry(YugabyteDBTypeRegistry yugabyteDBTypeRegistry) {
            this.yugabyteDBTypeRegistry = yugabyteDBTypeRegistry;
            return this;
        }

        @Override
        public Builder withSchema(YugabyteDBSchema schema) {
            this.schema = schema;
            return this;
        }
    }
