...
public class YbOutputMessageDecoder extends AbstractMessageDecoder {
    ...
    @Override
    public void processNotEmptyMessage(ByteBuffer buffer, ReplicationMessageProcessor processor, YugabyteDBTypeRegistry yugabyteDBTypeRegistry)
        ...
        switch (messageType) {
            ...
            case COMMIT:
                ...
                break;
            case RELATION:
                handleRelationMessage(buffer, yugabyteDBTypeRegistry);
                break;
            case INSERT:
                decodeInsert(buffer, yugabyteDBTypeRegistry, processor);
                break;
            case UPDATE:
                decodeUpdate(buffer, yugabyteDBTypeRegistry, processor);
                break;
            case DELETE:
                decodeDelete(buffer, yugabyteDBTypeRegistry, processor);
                break;
            case TRUNCATE:
                if (decoderContext.getConfig().truncateHandlingMode() == YugabyteDBConnectorConfig.TruncateHandlingMode.INCLUDE) {
                    decodeTruncate(buffer, yugabyteDBTypeRegistry, processor);
                }
                else {
    ...
    // CDCSDK the publication_names is empty
    @Override
     ...
     */
    private void handleRelationMessage(ByteBuffer buffer, YugabyteDBTypeRegistry yugabyteDBTypeRegistry) throws SQLException {
        ...
        for (short i = 0; i < columnCount; ++i) {
            ...
            final YugabyteDBType yugabyteDBType = yugabyteDBTypeRegistry.get(columnType);
    ...
    private List<io.debezium.relational.Column> getTableColumnsFromDatabase(YugabyteDBConnection connection, DatabaseMetaData databaseMetadata, TableId tableId)
            throws SQLException {
     ...
     *
     * @param buffer The replication stream buffer
     * @param yugabyteDBTypeRegistry The database type registry
    ...
    private static List<Column> resolveColumnsFromStreamTupleData(ByteBuffer buffer,
        ...
        for (short i = 0; i < numberOfColumns; ++i) {
            ...
            final YugabyteDBType columnType = yugabyteDBTypeRegistry.get(typeName);
